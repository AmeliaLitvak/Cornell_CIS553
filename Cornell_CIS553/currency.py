...
Author:Amelia Litval
Date:07/17/2025
"""
import introcs
APIKEY='KdHypsCxKpeSR0hYZUqohT6eNavqQAESENRosIY88eEe'
def before_space(s):
    """
...
def first_inside_quotes(s):
   ...
   precondition violation, since there are no double quotes inside the string.
    ...
    index=introcs.find_str(s,'"')
    index2=introcs.find_str(s,'"',index+1)
    result=s[index+1:index2]
    return result
...
def get_src(json):
    ...
    """
    assert type(json)==str, "assert error"
    index=introcs.find_str(json,'"src"')
    index2=introcs.find_str(json,',',index+5)
    result=first_inside_quotes(json[index+5:index2])
    return result
...
def has_error(json):
    ...
    index2=introcs.find_str(json,'}',index+7)
    error_message=first_inside_quotes(json[index+7:index2])
...
src='+src+'&dst='+dst+'&amt='+amt+'&key='+APIKEY
...
def exchange(src,dst,amt):
    ...
    output_dst=get_dst(json)
    dst_no_unit=before_space(output_dst)
    result=float(dst_no_unit)
    return result
